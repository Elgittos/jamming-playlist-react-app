...
function YourPlaylist({ selectedPlaylistId }) {
  ...
  return (
    <div className="w-full surface-panel rounded-2xl p-3 sm:p-4 h-full flex flex-col">
      <h2 className="text-base sm:text-lg font-bold text-white mb-2.5 sm:mb-3">Playlist Tracks</h2>

      {loading && (
        <div className="flex justify-center items-center py-8">
          <div className="animate-spin rounded-full h-10 w-10 border-b-2 accent-border" />
        </div>
      )}

      {error && !loading && (
        <div className="text-center py-8">
          <p className="accent-text-soft text-sm sm:text-base">{error}</p>
        </div>
      )}
      ...
      {!loading && !error && tracks.length > 0 && (
        <div className="space-y-1.5 max-h-[52vh] xl:max-h-[620px] overflow-y-auto pr-1.5 scrollbar-thin">
          {tracks.map((track, index) => (
            <button
              type="button"
              key={`${track.id}-${index}`}
              onClick={() => handleTrackClick(track.uri)}
              className="w-full rounded-lg p-2.5 border border-white/10 transition-all duration-300 cursor-pointer group flex items-center gap-2 text-left bg-white/5 hover:bg-white/10"
            >
              <div className="w-7 text-center text-gray-400 group-hover:text-white/80 text-xs sm:text-sm">
                {index + 1}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-white font-semibold text-sm sm:text-base truncate transition-colors">
                  {track.name}
                </h3>
              ...
              <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                <svg className="w-6 h-6 accent-text" fill="currentColor" viewBox="0 0 20 20">
      ...
      {!loading && !error && selectedPlaylistId && tracks.length === 0 && (
        <div className="text-center py-8">
          <p className="accent-text-soft text-sm sm:text-base">This playlist is empty</p>
